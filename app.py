...
# ── Logging — configure early so every module can inherit it ───────────────────
logging.basicConfig(
    level=logging.INFO,
...
def create_app(env=None):
    ...
    logger.info("[APP] Starting with config: %s", cfg.__name__)
    ...
    # Log mail config (mask the password)
    _log_mail_config(app)
    ...
    with app.app_context():
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Refund migration warning: %s", _mig_err)
            ...
            if current_user.is_authenticated and current_user.role == "theater_owner":
                ...
                if brand_id:
                    theaters = Theater.query.filter(
                        or_(Theater.brand_id == brand_id, Theater.owner_id == uid)
                    ).all()
                else:
                ...
                screens_count = (
                    Screen.query.filter(Screen.theater_id.in_(theater_ids)).count()
                    if theater_ids else 0
                )
                shows_count = (
                    Show.query.filter(Show.theater_id.in_(theater_ids)).count()
                    if theater_ids else 0
                )
                bookings_count = (
...
def _log_mail_config(app):
    """Log the effective mail configuration (password masked)."""
    logger.info(
        "[MAIL] Server=%s Port=%s TLS=%s Username=%s Password=%s DefaultSender=%s",
        app.config.get("MAIL_SERVER"),
        app.config.get("MAIL_PORT"),
        app.config.get("MAIL_USE_TLS"),
        app.config.get("MAIL_USERNAME"),
        "***SET***" if app.config.get("MAIL_PASSWORD") else "NOT SET ⚠️",
